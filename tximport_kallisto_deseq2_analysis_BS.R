...
samples <- read_csv(file.path('kallisto_Vc', 'kallisto_Vc_metadata_BS.csv'))
samples

files <- file.path('Desktop', 'kallisto_server_data', 'annotated_kallisto_alignments', samples$sample, 'abundance.h5')
files
...
write.csv(resordered, '/home/alexanderselvey/kallisto_tximport_deseq2_BS.csv')

res1 = subset(res,subset=res@listData[['padj']] < 0.05)
write.csv(res1, '/home/alexanderselvey/kallisto_tximport_deseq2_BS_sig.csv')
